...
Creates data/<DATASET>/processed/train_samples/
    <dataset>_isi_1024_align.json     — 1024 balanced samples for fine-tuning
    <dataset>_isi_full.json           — all samples (used as FULL_DATA_PATH)

Target format: "ID.EntityName" — matches history quadruplet format so the
model learns to generate a valid entity completion for the open query line.
"""
...
def _parse_answers(path: str, entity2id: dict) -> list:
    """Read test_ans.txt → list of 'ID.EntityName' strings."""
    answers = []
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) > 2:
                ...
                answers.append(f"{eid}.{name}")
    return answers


def _load_entity2id(proc_dir: Path) -> dict:
    path = proc_dir / "entity2id.json"
    if path.exists():
        with open(path, encoding="utf-8") as f:
            return json.load(f)
    return {}
...
def build_json_samples(dataset: str, data_root: str = "data",
    ...
    print(f"[samples] target example: {answers[0]}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--retrieve_type", "-t", default="isi")
